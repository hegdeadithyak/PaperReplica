...
class LSTMCell:
    ...
    def forward(self, x):
        i = self.sigmoid(np.dot(x, self.W_i) + np.dot(self.hidden_state, self.U_i) + self.b_i)
        f = self.sigmoid(np.dot(x, self.W_f) + np.dot(self.hidden_state, self.U_f) + self.b_f)
        o = self.sigmoid(np.dot(x, self.W_o) + np.dot(self.hidden_state, self.U_o) + self.b_o)
        c_hat = self.tanh(np.dot(x, self.W_c) + np.dot(self.hidden_state, self.U_c) + self.b_c)
